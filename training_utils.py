...
def space_loss(imgs1, imgs2, image_space=True, lpips_model=None):
    ...
    imgs1_kl = torch.nn.functional.softmax(imgs1.view(batch_size, -1), dim=1).view(
        imgs1.shape
    )
    imgs2_kl = torch.nn.functional.softmax(imgs2.view(batch_size, -1), dim=1).view(
        imgs2.shape
    )
    loss_imgs_kl = loss_kl(
        torch.log(imgs2_kl), imgs1_kl
    )  # D_kl(True=y1_imgs||Fake=y2_imgs)
    loss_imgs_kl = torch.where(
        torch.isnan(loss_imgs_kl), torch.full_like(loss_imgs_kl, 0), loss_imgs_kl
    )
    loss_imgs_kl = torch.where(
        torch.isinf(loss_imgs_kl), torch.full_like(loss_imgs_kl, 1), loss_imgs_kl
    )

    imgs1_cos = imgs1.view(-1)
    imgs2_cos = imgs2.view(-1)
    loss_imgs_cosine = 1 - imgs1_cos.dot(imgs2_cos) / (
        torch.sqrt(imgs1_cos.dot(imgs1_cos)) * torch.sqrt(imgs2_cos.dot(imgs2_cos))
    )  # [-1,1],-1:反向相反，1:方向相同
    ...
    loss_info = [
        [loss_imgs_mse_1.item(), loss_imgs_mse_2.item(), loss_imgs_mse_3.item()],
        loss_imgs_kl.item(),
