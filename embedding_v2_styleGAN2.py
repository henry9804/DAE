...
def train(tensor_writer=None, args=None, imgs_tensor=None):
    ...
    generator = StyleGAN2Generator(
        device, truncation=0.7, use_w=True, checkpoint_path=args.checkpoint_dir_GAN
    )
    ...
    for g in range(0, num // interval):
        ...
        for iteration in range(0, args.iterations):
            ...
            # loss AT1
            imgs_medium_1 = imgs1[
                :, :, :, imgs1.shape[3] // 8 : -imgs1.shape[3] // 8
            ]  # .detach().clone()
            imgs_medium_2 = imgs2[
                :, :, :, imgs2.shape[3] // 8 : -imgs2.shape[3] // 8
            ]  # .detach().clone()
            loss_medium, loss_medium_info = space_loss(
                imgs_medium_1, imgs_medium_2, lpips_model=loss_lpips
            )
            ...
            imgs_small_1 = imgs1[
                ...
                :,
                imgs1.shape[2] // 8
                + imgs1.shape[2] // 32 : -imgs1.shape[2] // 8
                - imgs1.shape[2] // 32,
                imgs1.shape[3] // 8
                + imgs1.shape[3] // 32 : -imgs1.shape[3] // 8
                - imgs1.shape[3] // 32,
            ]  # .detach().clone()
            imgs_small_2 = imgs2[
                :,
                :,
                imgs2.shape[2] // 8
                + imgs2.shape[2] // 32 : -imgs2.shape[2] // 8
                - imgs2.shape[2] // 32,
                imgs2.shape[3] // 8
                + imgs2.shape[3] // 32 : -imgs2.shape[3] // 8
                - imgs2.shape[3] // 32,
            ]  # .detach().clone()
            loss_small, loss_small_info = space_loss(
                imgs_small_1, imgs_small_2, lpips_model=loss_lpips
            )

            E_optimizer.zero_grad()
            loss_msiv = (
                loss_imgs + loss_medium * 0.125 * 3 + loss_small * 0.125 * 5
            )  # + w1.norm(p=2)*0.003 #这个比例看要优化的细节，dy的突出外围，眉毛的话突出中间
            ...
            E_optimizer.zero_grad()
            loss_msLv = (loss_w + loss_c1) * 0.01 + w1.norm(
                p=2
            ) * 0.0003  # 0.0003 0.0001 看要什么效果，重视重构效果就降低这个w1.norm(), 重视语意效果就提高
            ...
            if iteration > 1000:
                if loss_msiv_min > loss_msiv_flag * 1.03:
                    ...
                    torch.save(
                        w1,
                        resultPath1_2
                        + "/id%d-iter%d-norm%f-imgLoss-min%f.pt"
                        % (g, iteration, w1_flag, loss_msiv_flag),
                    )
                    test_img_min1 = (
                        torch.cat((imgs1[:batch_size], imgs2[:batch_size])) * 0.5 + 0.5
                    )
                    torchvision.utils.save_image(
                        test_img_min1,
                        resultPath1_1
                        + "/id%d_ep%d-norm%.2f-imgLoss-min%f.jpg"
                        % (g, iteration, w1_flag, loss_msiv_flag),
                        nrow=2,
                    )
                    with open(resultPath + "/loss_min.txt", "a+") as f:
                        print(
                            "ep%d_iter%d_minImg%.5f_wNorm%f"
                            % (g, iteration, loss_msiv_flag, w1_flag),
                            file=f,
                        )
                ...
                if w_norm_min > w1_flag * 1.05:
                    ...
                    torch.save(
                        w1,
                        resultPath1_2
                        + "/id%d-iter%d-norm-min%f-imgLoss%f.pt"
                        % (g, iteration, w1.norm(), loss_msiv_min.item()),
                    )
                    test_img_min2 = (
                        torch.cat((imgs1[:batch_size], imgs2[:batch_size])) * 0.5 + 0.5
                    )
                    torchvision.utils.save_image(
                        test_img_min2,
                        resultPath1_1
                        + "/id%d_ep%d-norm-min%.2f-imgLoss%f.jpg"
                        % (g, iteration, w1_flag, loss_msiv_flag),
                        nrow=2,
                    )
                    with open(resultPath + "/loss_min.txt", "a+") as f:
                        print(
                            "ep%d_iter%d_Img%.5f_wNorm-min%f"
                            % (g, iteration, loss_msiv_flag, w1_flag),
                            file=f,
                        )
            ...
            if iteration % 100 == 0:
                ...
                torchvision.utils.save_image(
                    test_img,
                    resultPath1_1
                    + "/id%d_ep%d-norm%.2f-imgLoss%f.jpg"
                    % (g, iteration, w1_flag, loss_msiv_flag),
                    nrow=2,
                )  # nrow=3
                ...
                for i, j in enumerate(w1):
                    torch.save(
                        j.unsqueeze(0),
                        resultPath1_2
                        + "/id%d-i%d-w%d-norm%f-imgLoss%f.pt"
                        % (g, i, iteration, w1_flag, loss_msiv_flag),
                    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--checkpoint_dir_GAN",
        default="./checkpoint/generators/stylegan2-ffhq-config-f.pt",
    )  # None  ./checkpoint/stylegan_v1/ffhq1024/ or ./checkpoint/stylegan_v2/stylegan2_ffhq1024.pth
    parser.add_argument("--config_dir", default=None)  # BigGAN needs it
    parser.add_argument(
        "--checkpoint_dir_E", default="./checkpoint/encoder/E_blur_stylegan2_ufl.pth"
    )  # E_blur(case2)_styleganv1_FFHQ_state_dict.pth, styleGANv1_EAE_ep115000.pth
    parser.add_argument("--img_dir", default="./real_imgs/")  # pt or directory
    parser.add_argument("--img_size", type=int, default=1024)
    parser.add_argument("--img_channels", type=int, default=3)  # RGB:3 ,L:1
    parser.add_argument("--z_dim", type=int, default=512)
    parser.add_argument(
        "--mtype", type=int, default=1
    )  # StyleGANv1=1, StyleGANv2=2, PGGAN=3, BigGAN=4
    parser.add_argument(
        "--start_features", type=int, default=16
    )  # 16->1024 32->512 64->256
    parser.add_argument(
        "--optimizeE", type=bool, default=True
    )  # if not, optimize W directly
    args = parser.parse_args()

    if not os.path.exists("./realimg_embedding_result"):
        os.mkdir("./realimg_embedding_result")
    ...
    if os.path.isdir(args.img_dir):  # img_file
        ...
        img_list.sort()
        img_tensor_list = [
            imgPath2loader(args.img_dir + i, size=args.img_size) for i in img_list
        ]
